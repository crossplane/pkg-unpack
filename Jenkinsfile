pipeline {
    ...
    environment {
        ...
        CODECOV_TOKEN = credentials('codecov-pkg-unpack')
    }
            ...
            steps {
                script {
                    if (env.CHANGE_ID != null) {
                        def json = sh (script: "curl -s https://api.github.com/repos/crossplane/pkg-unpack/pulls/${env.CHANGE_ID}", returnStdout: true).trim()
...
// NOTE(hasheddan): enable these steps when tests are added.
//
//        stage('Unit Tests') {
//            when {
//                expression {
